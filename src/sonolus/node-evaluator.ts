...
export type NodeEnvironment = { values?: Map<number, number>; random?: () => number; blocks?: SonolusBlocks; runtime?: SonolusPlayRuntime; entity?: EntityState }

const unaryMath: Record<string, (a: number) => number> = {
  Abs: Math.abs, Ceil: Math.ceil, Floor: Math.floor, Round: Math.round, Trunc: Math.trunc, Sin: Math.sin, Cos: Math.cos,
  Tan: Math.tan, Sinh: Math.sinh, Cosh: Math.cosh, Tanh: Math.tanh, Arccos: Math.acos, Arcsin: Math.asin, Arctan: Math.atan,
  Radian: (x) => x * Math.PI / 180, Degree: (x) => x * 180 / Math.PI, Negate: (x) => -x, Sign: Math.sign, Frac: (x) => x - Math.floor(x), Log: Math.log,
}
const binaryMath: Record<string, (a: number, b: number) => number> = {
  Add: (a, b) => a + b, Subtract: (a, b) => a - b, Multiply: (a, b) => a * b, Divide: (a, b) => b === 0 ? 0 : a / b,
  Mod: (a, b) => b === 0 ? 0 : a % b, Rem: (a, b) => b === 0 ? 0 : a % b, Power: Math.pow, Min: Math.min, Max: Math.max, Arctan2: Math.atan2,
}
...
export function evaluateNode(nodes: EngineDataNode[], index: number, env: NodeEnvironment = {}, stack = new Set<number>()): number {
  ...
  const cached = env.values?.get(index); if (cached !== undefined) return cached
  const node = nodes[index]; if ('value' in node) return node.value
  if (stack.has(index)) throw new Error(`Cyclic node graph at ${index}`)
  stack.add(index); const args = node.args.map((arg) => evaluateNode(nodes, arg, env, stack)); stack.delete(index)
  let result: number
  switch (node.func) {
    ...
    case 'Draw':
      if (args.length >= 11) env.runtime?.presentation.draw(args[0], [args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8]], args[9], args[10])
      result = 0; break
    case 'HasSkinSprite': result = 1; break
    case 'DestroyParticleEffect': case 'MoveParticleEffect': case 'Play': case 'PlayLooped': case 'PlayLoopedScheduled': case 'PlayScheduled': case 'StopLooped': case 'StopLoopedScheduled': case 'SpawnParticleEffect': case 'DebugLog': result = 0; break
    default: {
      const unary = unaryMath[node.func]; if (unary) { result = unary(args[0] ?? 0); break }
      const binary = binaryMath[node.func]; if (binary) { result = binary(args[0] ?? 0, args[1] ?? 0); break }
      throw new Error(`Unsupported Sonolus function: ${node.func}`)
    }
  }
  env.values?.set(index, result); return result
}

export function evaluateNodes(nodes: EngineDataNode[], indexes: number[], env: NodeEnvironment = {}): number[] { return indexes.map((index) => evaluateNode(nodes, index, env)) }
